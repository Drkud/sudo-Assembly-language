...
public class CU{
		...
		int r6=0;
		int r7=0;
		Picture display = new Picture(320,320);
	...
	private void def(String name ,int r2, int r3){
		...
		private void setd(int y ,int x, int val ){
			int pos  = 100;
			pos += x ;
			pos += y * 16 ;
			memory.set(pos,val);
		...
		public void postdisplay(){
			...
			for (int i = 0 ; i < 16 ; i++){
				for (int j = 0 ; j < 16 ; j++){
					...
					if (val > 0) colour = new Color	(255,0,0);
				...
				display= upscale(pic, 20);
				display.show();
				}

		private Picture upscale(Picture pic , int scale){
			double newWidth = (double) 16 * scale;
			double newHeight = (double) 16 * scale;
			for(int w =0; w < newWidth; w++){
				for(int h =0; h < newHeight; h++){
					int pos = (int) (w/scale) + (int) (h/scale);
					Color val = pic.get( (int) (w/scale) , (int) (h/scale) );
					display.set(w,h,val);
				}
			}
			return display;
